<script lang="ts">
  ...
  async function loadAll() {
    ...
    loadingE = true; errE = '';
    await Promise.all([
      api.get<UsersStats>(`/moderation/stats/users?${q}`)
        .then(d => { usersData = d; })
        .catch(e => { errU = e instanceof Error ? e.message : 'Fehler'; })
        .finally(() => { loadingU = false; }),
      api.get<EntriesStats>(`/moderation/stats/entries?${q}`)
  ...
  async function loadPrices(baseQ?: string) {
    ...
    loadingP = true; errP = '';
    try { pricesData = await api.get<PricesStats>(`/moderation/stats/prices?${p}`); }
    catch (e) { errP = e instanceof Error ? e.message : 'Fehler'; }
    finally { loadingP = false; }
  }

  onMount(async () => {
    lors = await api.get<LOR[]>('/moderation/stats/lors').catch(() => []);
    loadingC = true;
    changes = await api.get<PriceChange[]>('/moderation/stats/price-changes?limit=20').catch(() => []);
    loadingC = false;
    loadAll();
  });
